...
class StandardQueryData(BaseModel):
    ...
    task: Optional[str] = Field(
        default=None, description="Task type (qa, math, code, etc.)"
    )
...
class StandardRoutingData(BaseModel):
    ...
    model_scores: Optional[Dict[str, float]] = Field(
        default=None, description="Model performance scores"
    )
...
class GMTRouterInteraction(BaseModel):
    ...
    conversation: List[GMTRouterConversationTurn] = Field(
    ...
    encoder: Optional[str] = Field(
        default=None, description="PLM model name used for embeddings"
    )
    ...
    def validate_and_detect(self, data: Dict) -> tuple:
        ...
        if format_type == DataFormatType.UNKNOWN:
            return (
                False,
                format_type,
                "Unknown data format. Must be standard or GMTRouter format.",
            )
