...
# Import utils
from llmrouter.utils import setup_environment, generate_task_query, ProgressTracker
...
class LiteLLMRouterManager:
    ...
    def __init__(self, llm_data_path=None, llm_data_dict=None):
        ...
        if llm_data_dict:
            ...
            if os.path.exists(default_path):
                ...
                raise ValueError(
                    "No LLM data provided. Use llm_data_path or llm_data_dict"
                )
    ...
    def _load_model_config(self):
        ...
        self.allowed_models = [
            model for model in all_models if not model.endswith("_think")
        ]

        print(f"Total models in config: {len(all_models)}")
        print(
            f"Using {len(self.allowed_models)} non-think models: {self.allowed_models}"
        )
...
def process_single_query_model(args):
    ...
    try:
        ...
        if usage:
            ...
            completion_tokens = (
                len(response_text.split()) if isinstance(response_text, str) else 0
            )
        ...
        print(
            f"Error processing {base_row.get('task_name', 'unknown')} with {model_name}: {str(e)}"
        )
...
def generate_responses(base_df, router_manager, max_workers=100):
    ...
    print("Creating query-model combinations...")
    all_tasks = []
    ...
    print(
        f"Total tasks to process: {len(all_tasks)} ({len(base_df)} queries × {len(router_manager.allowed_models)} models)"
    )
    ...
    print(
        f"Success rate: {success_rate:.2f}% ({len(result_df) - error_count}/{len(result_df)})"
    )
...
def eval_perf(metric, prediction, ground_truth, task_name, task_id=None):
    ...
    # F1 score for partial matching
        ...
        if is_mbpp:
            ...
            code_match = re.search(
                r"\[BEGIN\](.*?)(?:\[DONE\]|$)", prediction, re.DOTALL | re.IGNORECASE
            )
            if code_match:
...
def evaluate_responses(df):
    ...
    # Process each row
    for idx, row in tqdm(
        df.iterrows(), total=len(df), desc="Evaluating responses", ncols=100
    ):
        try:
            ...
            ground_truth = (
                row.get("ground_truth") if "ground_truth" in row else row.get("gt")
            )
            task_name = row["task_name"]
            metric = row["metric"]
            model_name = row["model_name"]
            task_id = (
                row["task_id"] if has_task_id and not pd.isna(row["task_id"]) else None
            )
            ...
            if isinstance(prediction, str) and prediction.startswith("ERROR"):
                ...
                if task_name in CODE_TASK:
                    if (
                        ...
                        and not str(task_id).startswith("HumanEval")
                    ):
    ...
    for category, results in category_results.items():
        if results:
            print(
                f"{category.title()}: {np.mean(results):.4f} ({len(results)} samples)"
            )
...
def query_data_to_dataframe(query_data: List[Dict]) -> pd.DataFrame:
    ...
    for item in query_data:
        row = {
            ...
            "ground_truth": item[
                "ground_truth"
            ],  # Keep as ground_truth to match sample
            "metric": item["metric"],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="API Calling and Evaluation with LiteLLM Router"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run with limited samples for quick testing"
    )
    ...
    if args.test:
        ...
        print(
            f"Running in test mode with {len(query_data_train)} train and {len(query_data_test)} test samples..."
        )
    ...
    print(
        f"Tasks: {sorted(set(train_df['task_name'].unique()) | set(test_df['task_name'].unique()))}"
    )
    ...
    try:
        ...
        train_response_df = generate_responses(
            train_df, router_manager, max_workers=args.workers
        )
        train_evaluated_df = evaluate_responses(train_response_df)

        # Generate responses for test data
        print("\n=== Processing Test Data ===")
        test_response_df = generate_responses(
            test_df, router_manager, max_workers=args.workers
        )
        ...
        embedding_dict, train_final, test_final = (
        ...
        print(
            f"  - Average train performance: {train_evaluated_df['performance'].mean():.4f}"
        )
        print(
            f"  - Average test performance: {test_evaluated_df['performance'].mean():.4f}"
        )
