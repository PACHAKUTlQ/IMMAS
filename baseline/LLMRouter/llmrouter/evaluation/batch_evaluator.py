...
def _eval_exact_match(
    prediction: str, ground_truth: str, normal_method: str = "", **kwargs
) -> float:
    """Exact match evaluation metric"""
    return float(
        exact_match_score(prediction, ground_truth, normal_method=normal_method)
    )
...
def evaluate_batch(
    data: List[Dict[str, Any]], default_metric: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    for item in data:
        ...
        try:
            ...
            results.append(result)
            print(
                f"Warning: Evaluation failed for item with metric '{metric_name}': {e}"
            )
