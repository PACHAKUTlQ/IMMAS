...
try:
    from custom_routers.auctionrouter.router import AuctionRouter
except Exception:
    AuctionRouter = None
...
if AuctionRouter is not None:
    ROUTER_REGISTRY["auctionrouter"] = AuctionRouter
    ...
    for router_name, router_class in plugin_registry.discovered_routers.items():
        ...
        if isinstance(router_class, tuple):
            ROUTER_REGISTRY[router_name] = router_class[
                0
            ]  # Only router class for inference
        else:
            ROUTER_REGISTRY[router_name] = router_class
        ...
        if file_ext == ".json":
            ...
            if isinstance(data, list):
                if all(isinstance(item, str) for item in data):
                    ...
                    raise ValueError(
                        "JSON file must contain list of strings or list of dicts with 'query' field"
                    )
            else:
            ...
            for line in f:
                ...
                if line:
                    ...
                    if isinstance(obj, dict) and "query" in obj:
                        ...
                        raise ValueError(
                            "JSONL line must be dict with 'query' field or string"
                        )
            return queries
...
def save_results_to_file(
    results: List[Dict[str, Any]], output_path: str, output_format: str = "json"
):
    """
  ...
  python router_inference.py --router knnrouter --config config.yaml --query "Hello" --route-only
    ...
    if args.query:
        ...
        try:
            ...
            if args.verbose:
                print(
                    f"Loaded {len(queries)} queries from {args.input}", file=sys.stderr
                )
        except Exception as e:
