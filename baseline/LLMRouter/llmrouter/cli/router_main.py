...
def chat_command(args):
    ...
    try:
        ...
        print(
            "Error: gradio is required for chat interface. Install it with:",
            file=sys.stderr,
        )
    ...
    def predict_with_router(message, history, temperature, mode, top_k):
        return predict(
            message, history, router_instance, args.router, temperature, mode, top_k
        )
...
def list_routers_command(args):
    ...
    for router_name in sorted(ROUTER_TRAINER_REGISTRY.keys()):
        ...
        print(
            f"  {router_name:25s} - {router_class.__name__} / {trainer_class.__name__}"
        )
