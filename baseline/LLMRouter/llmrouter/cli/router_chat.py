...
def prepare_query_retrieval(message: str, history: list, top_k: int = 3) -> str:
    ...
    try:
        ...
        historical_norms = historical_embeddings / (
            np.linalg.norm(historical_embeddings, axis=1, keepdims=True) + 1e-8
        )
        ...
        print(
            f"Warning: Retrieval mode failed, falling back to current query only: {e}"
        )
        return message


def prepare_query_by_mode(
    message: str, history: list, mode: str, top_k: int = 3
) -> str:
    """
...
def load_router(
    router_name: str, config_path: str, load_model_path: Optional[str] = None
):
    """
...
def predict(
    ...
    if router_name_lower in ROUTERS_REQUIRING_SPECIAL_ARGS:
        try:
            ...
            api_base = hparam.get("api_base") or getattr(
                router_instance, "api_base", None
            )
    ...
    try:
        ...
        if hasattr(router_instance, "llm_data") and router_instance.llm_data:
            if model_name in router_instance.llm_data:
                ...
                api_model_name = router_instance.llm_data[model_name].get(
                    "model", model_name
                )
...
def create_interface(router_instance, router_name: str, args):
    ...
    def predict_with_router(message, history, temperature, mode, top_k):
        return predict(
            message, history, router_instance, router_name, temperature, mode, top_k
        )

    # Use gr.themes.Base() to minimize default styling interference, but Default is also fine with our overrides
    with gr.Blocks(
        css=CUSTOM_CSS, theme=gr.themes.Default(), title="LLMRouter"
    ) as demo:
        # Minimal top bar
        gr.HTML(f"""
            <div class="top-bar">
        ...
        def bot_response(chat_history, temperature, mode, top_k):
            ...
            full_response = predict_with_router(
                user_msg, history_for_router, temperature, mode, top_k
            )
        ...
        def retry_last(chat_history, temperature, mode, top_k):
            ...
            full_response = predict_with_router(
                last_user_msg, history_for_router, temperature, mode, top_k
            )
            partial = ""
            for char in full_response:
                partial += char
