...
def format_api_request_with_task(
    query_text, task_name, api_endpoint, model_name, api_model_name, choices=None
):
    """
    ...
    if task_name:
        try:
            sample_data = {"query": query_text, "choices": choices}
            ...
            print(
                f"Warning: Failed to format query with task '{task_name}': {e}. Using original query."
            )
