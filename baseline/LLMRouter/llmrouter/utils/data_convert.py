...
def calculate_cost(input_tokens: int, output_tokens: int, model_name: str) -> float:
    ...
    return (input_tokens / 1000) * default_input_price + (
        output_tokens / 1000
...
def call_llm_for_ability(
    query: str, task_name: str, api_key: str, api_base: str = None
) -> str:
...
def convert_single_record(
    old_record: Dict, index: int, use_llm: bool = False, api_key: str = None
) -> Dict:
    ...
    new_record = {
        ...
        "data_source": old_record.get(
            "task_name", "default"
        ),  # Use task_name as data_source
        "prompt": generate_prompt(question),
        "ability": determine_ability(old_record.get("task_name", ""), question),
        "reward_model": {"ground_truth": {"target": golden_answers}, "style": "rule"},
        "extra_info": {"index": index, "split": "test"},
    }

    # If need to use LLM to generate ability
    if use_llm and api_key:
        new_record["ability"] = call_llm_for_ability(
            question, old_record.get("task_name", ""), api_key
        )
...
def process_batch(
    records: List[Dict], start_index: int, use_llm: bool = False, api_key: str = None
) -> List[Dict]:
    """Process records in batch"""
    results = []
    for i, record in enumerate(records):
        try:
            new_record = convert_single_record(
                record, start_index + i, use_llm, api_key
            )
...
def convert_data(
    ...
    if use_llm:
        print(
            f"Using LLM API to generate missing fields (using {max_workers} parallel workers)..."
        )
        # Use API key
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for i in range(0, len(records), batch_size):
                    ...
                    future = executor.submit(
                        process_batch, batch, start_index + i, use_llm, api_key
                    )
                    futures.append(future)

                for future in tqdm(
                    as_completed(futures), total=len(futures), desc="Converting records"
                ):
        ...
        for i, record in enumerate(tqdm(records, desc="Converting records")):
            try:
                new_record = convert_single_record(
                    record, start_index + i, use_llm, None
                )
...
def convert_train_data(
    input_file: str, output_file: str, start_line: int = None, end_line: int = None
):
    ...
    # Convert each group
    converted_records = []
    for q_id, (query, group_records) in enumerate(
        tqdm(grouped_records.items(), desc="Converting records")
    ):
        try:
            ...
            if perf_list:
                ...
                best_llm_name = (
                    llm_name_list[best_perf_idx]
                    if best_perf_idx < len(llm_name_list)
                    else ""
                )
            else:
                best_perf_idx = 0
...
def merge_train_test(test_file: str, train_file: str, output_file: str):
        ...
        with open(train_file, "r", encoding="utf-8") as f:
            try:
                ...
                if isinstance(train_data, list):
                    ...
                    print(
                        "Warning: train file format is incorrect, should be JSON array"
                    )
            except json.JSONDecodeError as e:
    ...
    print(
        f"Total {len(merged_records)} merged records (test: {len(test_records)}, train: {len(train_records)})"
    )
...
def main():
    ...
    parser.add_argument(
        "--end-line", type=int, default=None, help="End line number (exclusive)"
    )
    ...
    if args.mode == "merge":
        ...
        if not args.test_file or not args.train_file:
            print(
                "Error: merge mode requires --test-file and --train-file to be specified"
            )
            sys.exit(1)
        merge_train_test(
