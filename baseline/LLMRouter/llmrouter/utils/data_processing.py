...
def process_final_data(df_all):
    ...
    embedding_df = df_all.groupby(
        ["task_name", "query", "gt", "metric"], as_index=False
    ...
    for _, row in embedding_df.iterrows():
        ...
        if isinstance(query_embedding, np.ndarray):
            embedding_dict[embedding_id] = torch.tensor(
    ...
    df_train_indexed.to_json(
        "default_routing_train_data.jsonl",
        orient="records",
        lines=True,
        force_ascii=False,
    )
    df_test_indexed.to_json(
...
def process_unified_embeddings_and_routing(
    ...
    embedding_results = generate_embeddings_for_data(
        all_queries, "Generating unified embeddings"
    )
    ...
    for idx, (query_item, (_, embedding, success)) in enumerate(
        zip(all_queries, embedding_results)
    ):
        if not success:
    ...
    print(
        f"✅ Saved {len(embedding_dict)} unified embeddings to {embedding_output_path}"
    )
    ...
    def add_embedding_id(df):
        ...
        for _, row in df.iterrows():
            ...
            key = (row["task_name"], row["query"], gt_value, row["metric"])
    ...
    def format_routing_record(row):
        ...
        required_fields = {"user_id": None, "fig_id": None}
...
def generate_embeddings_for_data(data, desc="Generating embeddings"):
    ...
    with ThreadPool(100) as p:
        for r in tqdm(
            p.imap_unordered(parallel_embedding_task, task_args),
            total=len(task_args),
            desc=desc,
            ncols=100,
        ):
    ...
    print(
        f"Embedding generation complete: Success: {len(ret_1) - fail_count}, Fail: {fail_count}"
    )
