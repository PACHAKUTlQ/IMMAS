...
def exact_match_score(
    prediction: str, ground_truth: str, normal_method: str = ""
) -> bool:
    """
    ...
    if normal_method == "mc":
        return (
            ...
            in normalize_answer(prediction, normal_method=normal_method).strip().lower()
        )
    return normalize_answer(
        prediction, normal_method=normal_method
...
def calculate_task_performance(
    ...
    try:
        ...
        if metric == "em":
            ...
            return float(
                exact_match_score(prediction, ground_truth, normal_method="mc")
            )
        elif metric == "cem":
...
def model_prompting(
    ...
    base_url = os.environ.get("OPENAI_API_BASE") or os.environ.get("NVIDIA_API_BASE")
