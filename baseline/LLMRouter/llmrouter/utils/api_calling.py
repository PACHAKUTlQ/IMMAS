...
def _get_api_key(
    ...
    if isinstance(api_keys, dict):
        ...
        if not service_keys:
            raise ValueError(
                f"No API keys available for service '{matching_service}' in API_KEYS dict"
            )

        # Check if this is a local endpoint (localhost/127.0.0.1) with empty string key
        # Allow empty string for local providers
        is_local_endpoint = (
            "localhost" in api_endpoint.lower()
            or "127.0.0.1" in api_endpoint
    ...
    # Handle list format (legacy)
        ...
        if not is_local_endpoint:
            ...
            if not api_keys:
                raise ValueError(
                    "No valid API keys provided. Empty strings are only allowed for localhost endpoints."
                )
    ...
    if is_batch:
        ...
        _api_key_counters[cache_key] = (_api_key_counters[cache_key] + 1) % len(
            keys_to_use
        )
    ...
    for idx, req in enumerate(requests):
        ...
        try:
            ...
            if not api_key_for_litellm:
                ...
                is_local = (
                    "localhost" in req["api_endpoint"].lower()
                    or "127.0.0.1" in req["api_endpoint"]
                    or req["api_endpoint"].startswith("http://127.0.0.1")
                    or req["api_endpoint"].startswith("http://localhost")
                )
                if is_local:
