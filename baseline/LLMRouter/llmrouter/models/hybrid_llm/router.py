...
class HybridLLMRouter(MetaRouter):
    ...
    def __init__(self, yaml_path: str):
        ...
        # -------------------------------
        (self.query_embedding_list, self.router_label_list) = (
    ...
    def _resolve_small_large(self):
        ...
        if len(available) < 2:
            raise ValueError(
                "[HybridLLMRouter] Need at least 2 models (size ends with B)."
            )

        sorted_models = sorted(
            available, key=lambda m: parse_size(self.llm_data[m].get("size", "0B"))
        )
    ...
    def _create_training_dataset(self):
        ...
        if pair_df.empty:
            raise ValueError(
                "[HybridLLMRouter] No routing data found for small/large pair."
            )
        ...
        if pivot.empty:
            raise ValueError(
                "[HybridLLMRouter] No complete rows with both small & large scores."
            )
