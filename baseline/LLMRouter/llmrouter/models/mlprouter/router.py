...
class MLPRouter(MetaRouter):
    ...
    def __init__(self, yaml_path: str):
        ...
        embeddings_tensor = torch.stack(
            [self.query_embedding_data[i] for i in query_embedding_id]
        )
        ...
        self.label_indices = torch.tensor(
            [self.model_to_idx[m] for m in self.model_name_list], dtype=torch.long
        )
    ...
    def _load_mlp_model(self, device: str = "cpu"):
        ...
        if isinstance(state_dict, dict) and any(
            k.startswith("layers") for k in state_dict.keys()
        ):
            # PyTorch state dict
            hidden_layer_sizes = self.hparam.get("hidden_layer_sizes", [128, 64])
            activation = self.hparam.get("activation", "relu")
