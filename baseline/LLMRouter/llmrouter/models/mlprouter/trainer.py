...
class MLPTrainer(BaseTrainer):
    ...
    def __init__(self, router, optimizer=None, device="cpu"):
        ...
        self.ini_model_path = os.path.join(
            project_root, router.cfg["model_path"]["ini_model_path"]
        )
        ...
        print(
            f"[MLPTrainer] Model: {hidden_layer_sizes}, lr={self.lr}, epochs={self.epochs}, batch_size={self.batch_size}"
        )
    ...
    def train(self):
        ...
        if (
            self.ini_model_path
            and os.path.exists(self.ini_model_path)
            and self.ini_model_path.endswith(".pkl")
        ):
            try:
                ...
                if isinstance(state_dict, dict) and any(
                    ...
                    print(
                        f"[MLPTrainer] Loaded initial model from {self.ini_model_path}"
                    )
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 10 == 0 or epoch == self.epochs - 1:
                ...
                print(
                    f"[MLPTrainer] Epoch {epoch + 1}/{self.epochs} - Loss={avg_loss:.6f}, Accuracy={accuracy:.4f}"
                )
