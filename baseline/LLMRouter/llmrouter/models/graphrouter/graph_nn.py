...
class EncoderDecoderNet(torch.nn.Module):
    ...
    def __init__(self, query_feature_dim, llm_feature_dim, hidden_features, in_edges=1):
        ...
        self.model_align = FeatureAlign(
            query_feature_dim, llm_feature_dim, hidden_features
        )
    ...
    def forward(
        ...
        edge_weight_mask = F.leaky_relu(
            self.edge_mlp(edge_weight_mask.reshape(-1, self.in_edges))
        )
        ...
        x = F.leaky_relu(
            self.bn1(
                self.encoder_conv_1(x_ini, edge_index_mask, edge_attr=edge_weight_mask)
            )
        )
        x = self.bn2(
            self.encoder_conv_2(x, edge_index_mask, edge_attr=edge_weight_mask)
        )
...
class FormData:
    ...
    def formulation(
        ...
        edge_index = torch.tensor([org_node, des_node], dtype=torch.long).to(
            self.device
        )

        # Edge features only use performance
        edge_weight = (
            torch.tensor(edge_feature, dtype=torch.float).reshape(-1, 1).to(self.device)
        )
...
class GNNPredictor:
    ...
    def train_validate(self, data, data_validate):
        ...
        for epoch in range(train_epochs):
            ...
            for _ in range(batch_size):
                ...
                mask = torch.where(
                    mask & random_mask, torch.tensor(False, dtype=torch.bool), mask
                )
                mask = mask.bool()
                ...
                loss = self.criterion(
                    predicted_edges.reshape(-1), data.label[mask].reshape(-1)
                )
                loss_mean += loss
            ...
            with torch.no_grad():
                ...
                value_validate = data_validate.edge_attr[mask_validate].reshape(
                    -1, self.num_llms
                )
                ...
                if epoch % 10 == 0:
                    print(
                        f"Epoch {epoch}: train_loss={loss_mean:.4f}, val_result={result_validate:.4f}"
                    )
    ...
    def predict(self, data):
        ...
        edge_can_see = torch.logical_or(data.train_mask, data.valide_mask).to(
            torch.bool
        )
