...
class GraphRouter(MetaRouter):
    ...
    def _prepare_training_data(self):
        ...
        for query in unique_queries:
            query_data = self.routing_data_train[
                self.routing_data_train["query"] == query
            ]
    ...
    def route_single(self, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get query embedding
        query_embedding = (
            get_longformer_embedding(query["query"]).numpy().reshape(1, -1)
        )
        ...
        all_query_embeddings = np.vstack(
            [self.query_embedding_list, query_embedding_normalized]
        )
    ...
    def route_batch(
        ...
        all_query_embeddings = np.vstack(
            [self.query_embedding_list, test_embeddings_normalized]
        )
        ...
        for i, row in enumerate(query_data):
            ...
            if (
                ...
                and self.llm_data
                and model_name in self.llm_data
            ):
    ...
    def get_training_data(self):
        ...
        return self._build_graph_data(
            self.query_embedding_list, self.performance_list, is_train=True
        )
