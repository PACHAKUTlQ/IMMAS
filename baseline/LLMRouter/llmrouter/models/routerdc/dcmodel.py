...
class RouterModule(nn.Module):
    ...
    def compute_sample_llm_loss(
        self, x: torch.Tensor, index_true: torch.Tensor, top_k: int, last_k: int
    ) -> torch.Tensor:
        """
        ...
        last_index_true, negtive_index = index_true.topk(
            k=last_k, largest=False, dim=-1
        )
    ...
    def compute_sample_sample_loss_with_task_tag(
        ...
        for dataset_id in dataset_ids:
            ...
            index_of_negtive_indexs = random.sample(
                range(0, len(negtive_indexs)), last_k2
            )
