...
class DCTrainer(BaseTrainer):
    ...
    def __init__(self, router, device=None):
        ...
        self.save_dir = os.path.join(
            router.project_root, os.path.dirname(save_model_path)
        )
    ...
    def loss_func(self, outputs, batch) -> torch.Tensor:
        ...
        loss = self.router.model.compute_sample_llm_loss(
            x=scores, index_true=true_scores, top_k=self.top_k, last_k=self.last_k
        )
        ...
        if self.sample_loss_weight > 0:
            ...
            sample_sample_loss = (
                self.router.model.compute_sample_sample_loss_with_task_tag(
    ...
    def _evaluate_and_save(self, step: int):
        ...
        # Compute averages
        if test_results:
            test_average = sum([r[1] for r in test_results.values()]) / len(
                test_results
            )
