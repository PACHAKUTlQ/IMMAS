...
def aggregate_by_query(records: List[Dict]) -> List[Dict]:
    ...
    for query, group_records in grouped.items():
        ...
        if llm_name_list:  # Only add if we have at least one LLM response
            aggregated.append(
                {"query": query, "llm_name_list": llm_name_list, "perf_list": perf_list}
            )
...
def convert_format(input_data: List[Dict]) -> List[Dict]:
    ...
    for item in input_data:
        ...
        # Build new format
        new_item = {"question": item["query"], "scores": scores}
...
def add_clusters(
    data: List[Dict], n_clusters: int = 3, random_state: int = 42
) -> List[Dict]:
    """
