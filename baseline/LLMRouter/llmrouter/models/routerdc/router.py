...
class DCRouter(MetaRouter):
    ...
    def __init__(self, yaml_path: str):
        ...
        self.tokenizer = DebertaV2Tokenizer.from_pretrained(
            backbone_model, truncation_side="left", padding=True
        )
    ...
    def _prepare_data(self):
        ...
        train_data_raw = os.path.join(
            self.project_root, data_path_config["routing_data_train"]
        )
        test_data_raw = os.path.join(
            self.project_root, data_path_config["routing_data_test"]
        )
        ...
        self.train_data_processed = preprocess_data(
            input_path=train_data_raw, add_cluster_id=True, n_clusters=n_clusters
        )

        # Preprocess test data
        self.test_data_processed = preprocess_data(
            input_path=test_data_raw, add_cluster_id=False, n_clusters=n_clusters
        )
    ...
    def route_batch(
        ...
        save_dir = os.path.join(
            self.project_root,
            os.path.dirname(self.cfg["model_path"]["save_model_path"]),
        )
            ...
            for row in query_data:
                ...
                if row_task_name:
                    try:
                        ...
                        formatted_query = generate_task_query(
                            row_task_name, sample_data
                        )
                ...
                if (
                    ...
                    and self.llm_data
                    and predicted_llm in self.llm_data
                ):
                    api_model_name = self.llm_data[predicted_llm].get(
                        "model", predicted_llm
                    )
                ...
                for batch_data in test_dataloader:
                    ...
                    for i in range(len(predictions)):
                        if sample_idx < len(self.test_dataset.data):
                            ...
                            predicted_llm = self.test_dataset.router_node[
                                predicted_llm_idx
                            ]
                            ...
                            try:
                                result = call_api(
                                    request, max_tokens=1024, temperature=0.7
                                )
                                response = result.get("response", "")
                                prompt_tokens = result.get("prompt_tokens", 0)
                                completion_tokens = result.get("completion_tokens", 0)
