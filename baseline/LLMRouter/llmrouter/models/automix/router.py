...
def detect_small_large_models(llm_data_path: str) -> Tuple[str, str]:
    ...
    print(
        f"✅ Auto-detected small model: {smallest_name} ({smallest_size}B) -> {smallest_engine}"
    )
    print(
        f"✅ Auto-detected large model: {largest_name} ({largest_size}B) -> {largest_engine}"
    )
...
class AutomixRouter(MetaRouter):
    ...
    def _prepare_data(self):
        ...
        if not train_path or not test_path:
            raise ValueError(
                "Config must specify 'routing_data_train' and 'routing_data_test' in data_path section"
            )
        ...
        if "gt" in train_df.columns and "gt" in test_df.columns:
            ...
            ground_truth_col = next(
                (c for c in candidates if c in train_df.columns), "gt"
            )
        ...
        results_slm = run_solver_job(
            merged_df, prepare_row, engine_small, max_workers=max_workers
        )
        results_llm = run_solver_job(
            merged_df, prepare_row, engine_large, max_workers=max_workers
        )
        ...
        if "metric" in merged_df.columns:
            is_multi_choice = (
                merged_df["metric"]
                .str.contains("mc|multiple_choice", case=False, na=False)
                .any()
            )
            ...
            if len(sample_gt) > 0:
                is_multi_choice = (
                    sample_gt.astype(str)
                    .str.strip()
                    .str.match(r"^[A-D]$", na=False)
                    .any()
                )
        ...
        if is_multi_choice:
            ...
            for model_name in model_names:
                ...
                merged_df[f"{model_name}_pred_option"] = merged_df[pred_col].apply(
                    extract_option_from_pred
                )
                ...
                merged_df[f1_col] = merged_df.apply(
                    ...
                    if (
                        ...
                        and str(r.get(ground_truth_col, "")).strip().upper()
                        == str(r.get(f"{model_name}_pred_option")).strip().upper()
                    )
                    else 0.0,
                    axis=1,
                )
        ...
        merged_df[self.verifier_column] = merged_df["slm_ver"].apply(
            compute_fraction_correct
        )

        # Categorize rows
        merged_df = categorize_rows(
            merged_df, slm_column=self.slm_column, llm_column=self.llm_column
        )

        # Split back into train and test
        train_df = (
            merged_df[merged_df["split"] == "train"].copy().reset_index(drop=True)
        )
        ...
        for col in required_cols:
            if col not in train_df.columns:
                raise ValueError(
                    f"Training data missing required column after processing: {col}"
                )
            if col not in test_df.columns:
                raise ValueError(
                    f"Test data missing required column after processing: {col}"
                )
    ...
    def _build_method(name: str, num_bins: int):
        ...
        if name not in mapping:
            raise ValueError(
                f"Unsupported routing method: {name}. Available: {list(mapping.keys())}"
            )
    ...
    # ------------------------------------------------------------------
    def route_batch(
        self, batch: Optional[Any] = None, task_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if batch is not None:
            ...
            if hasattr(self, "query_data_test") and self.query_data_test is not None:
                ...
                print(
                    "Warning: No batch provided and no test data available for batch routing."
                )
                return []
        ...
        for row in query_data:
            ...
            verification_score = (
                compute_fraction_correct(ver_results[0]) if ver_results else 0.0
            )
            ...
            if route_to_llm:
                results_llm = run_solver_job(
                    query_df, prepare_row, self.engine_large, max_workers=1
                )
            ...
            if row_task_name:
                try:
                    ...
                    print(
                        f"Warning: Failed to format query with task '{row_task_name}': {e}. Using original query."
                    )
    ...
    def route_single(self, sample: Any) -> Dict[str, Any]:
        ...
        results_slm = run_solver_job(
            query_df, prepare_row, self.engine_small, max_workers=1
        )
