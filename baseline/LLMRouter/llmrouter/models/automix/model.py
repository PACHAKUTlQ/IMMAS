...
class AutomixModel(nn.Module):
    ...
    def get_slm_llm_slope_perf(
        self, data: pd.DataFrame, costs=None, verifier_cost=None
    ):
        """
    ...
    def train_routing(
        ...
        for param in self.method.generate_points(
            ...
            to_retry = self.method.run(
                data, param, verifier_column=self.verifier_column
            )
        ...
        if not valid_scores:
            ...
            best_key = (
                safe_max_key(valid_scores)
                if valid_scores
                else list(thresh_dic.keys())[0]
            )
