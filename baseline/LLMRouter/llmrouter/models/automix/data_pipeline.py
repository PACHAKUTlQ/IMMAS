...
def get_client(base_url="", api_key="", max_retries=2, timeout=60):
    ...
    if (
        ...
        or _cached_base_url != base_url
        or _cached_api_key != api_key
    ):
        print(
            f"[DEBUG] get_client: Creating new client - "
        ...
        if api_key:
            ...
            for var in ["OPENAI_API_KEY", "NVIDIA_API_KEY", "NVAPI_KEY", "API_KEYS"]:
                if os.environ.get(var):
                    ...
                    break
            print(
                f"[DEBUG] init_providers: API key from {used_var}, "
...
def get_llm_response_via_api(
    ...
    if not api_key:
        print(
            "[ERROR] API key not found. Set OPENAI_API_KEY/NVIDIA_API_KEY/NVAPI_KEY/API_KEYS"
        )
        return None, 0

    # Debug: Print API key info
    print(
        f"[DEBUG] get_llm_response_via_api: Using api_key={api_key[:10]}...{api_key[-4:] if len(api_key) > 14 else ''}, "
    ...
    if _cached_api_key != api_key:
        print(
            f"[ERROR] Client API key mismatch! Expected: {api_key[:10]}..., "
        ...
        _cached_client = OpenAI(
            base_url=base_url, api_key=api_key, max_retries=2, timeout=60
        )
        _cached_base_url = base_url
        _cached_api_key = api_key
        print("[DEBUG] Force recreated client with correct API key")
    ...
    while trials > 0:
        ...
        try:
            ...
            if (
                ...
                print(
                    f"[ERROR] Auth failed! Using api_key={api_key[:10]}...{api_key[-4:] if len(api_key) > 14 else ''}, "
                    f"base_url={base_url}, model={LLM_MODEL}"
                )
...
def compute_f1(
    ...
    return f1_score_single(prediction, ground_truth if ground_truth is not None else "")
...
def run_verification(
    ...
    if question_col not in df.columns:
        raise ValueError(
            f"DataFrame must contain either 'question' or 'query' column. Found columns: {list(df.columns)}"
        )
    ...
    if none_count > 0:
        print(
            f"[DEBUG] run_verification: {none_count}/{len(results)} verification results are None"
        )
...
def compute_fraction_correct(
    lst: Union[List[str], List[Union[str, None]], None],
) -> float:
    """
    ...
    if lst is None or not lst:
        print("[DEBUG] compute_fraction_correct: Received None or empty list")
        return 0.0
    ...
    if none_count > 0:
        print(
            f"[DEBUG] compute_fraction_correct: Filtered out {none_count} None values from {len(lst)} items"
        )
    if not valid_items:
        print(
            "[DEBUG] compute_fraction_correct: No valid items after filtering, returning 0.0"
        )
        return 0.0

    # Convert items to strings if they aren't already
    str_items = [
        str(item) if not isinstance(item, str) else item for item in valid_items
    ]
    ...
    correct_count = sum(
        [
            ...
            if "the ai generated answer is correct" in item.lower()
        ]
    )
...
def categorize_rows(
    df: pd.DataFrame, slm_column: str = "slm_f1", llm_column: str = "llm_f1"
) -> pd.DataFrame:
    """
    ...
    conditions = [
        (df[slm_column] <= df[llm_column]) & (df[slm_column] != df[llm_column]),
