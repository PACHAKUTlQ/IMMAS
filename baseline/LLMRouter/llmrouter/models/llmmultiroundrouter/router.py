...
class LLMMultiRoundRouter(MetaRouter):
    ...
    def route_single(self, query):
        ...
        final_answer = self._aggregate_responses(
            original_query, sub_queries, sub_responses, task_name
        )
        ...
        ground_truth = (
            query_dict.get("ground_truth")
            or query_dict.get("gt")
            or query_dict.get("answer")
        )
    ...
    def route_batch(
        ...
        for row in query_data:
            ...
            routing_result = self.route_single(
                {"query": query_text_for_routing, "task_name": row_task_name}
            )
    ...
    def _build_decomp_route_prompt(self) -> str:
        ...
        model_list = (
            ", ".join(self.llm_data.keys())
            if hasattr(self, "llm_data") and self.llm_data
            else "Available models"
        )

        template = load_prompt_template("agent_decomp_route")
        prompt = template.format(
            model_list=model_list, model_descriptions=self.model_descriptions
        )
        return prompt
    ...
    def _initialize_local_llm(self):
        ...
        try:
            ...
            print(
                f"⚠️  Failed to initialize local LLM: {e}. Will use API calls instead."
            )
    ...
    def _parse_model_name(self, response: str) -> str:
        ...
        if hasattr(self, "llm_data") and self.llm_data:
            ...
            for model_key, model_info in self.llm_data.items():
                ...
                if (
                    model_key_lower in response_lower
                    or model_api_name in response_lower
                ):
    ...
    def _decompose_and_route(self, query: str) -> List[tuple]:
        ...
        if self.use_local_llm:
            ...
            service = None
            if (
                hasattr(self, "llm_data")
                and self.llm_data
                and self.base_model in self.llm_data
            ):
                base_api_endpoint = self.llm_data[self.base_model].get(
...
<answer>A</answer>"""
            ...
            agg_prompt = self.DECOMP_COT_PROMPT.format(
                query=original_query, info=input_info
            )
