...
class GMTRouterTrainer(BaseTrainer):
    ...
    def __init__(self, router, optimizer=None, device=None):
        ...
        self.checkpoint_root = os.path.join(
            project_root, checkpoint_config.get("root", "models")
        )
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if epoch % self.eval_every == 0 or epoch == self.epochs:
                ...
                print(
                    f"Epoch {epoch}/{self.epochs} - Train Loss: {train_loss:.4f}, Train {self.objective.upper()}: {train_metric:.4f}"
                )
    ...
    def _initialize_model(self, train_data: Dict):
        ...
        print(
            f"Model initialized with metadata: {len(metadata[0])} node types, {len(metadata[1])} edge types"
        )
    ...
    def _train_epoch(self, train_data: Dict, epoch: int) -> Tuple[float, float]:
        ...
        if "x_dict" in train_data:
            ...
            edge_index_dict = {
                k: v.to(self.device) for k, v in train_data["edge_index_dict"].items()
            }
        else:
            # Simplified data
            x_dict = {
        ...
        for pair_idx in sampled_pairs:
            ...
            query_ids = torch.tensor(
                [winner["query_id"], loser["query_id"]],
                dtype=torch.long,
                device=self.device,
            )
            llm_ids = torch.tensor(
                [winner["llm_id"], loser["llm_id"]],
                dtype=torch.long,
                device=self.device,
            )
    ...
    def _validate(self, val_data: Dict) -> Tuple[float, float]:
        ...
        if "x_dict" in val_data:
            ...
            edge_index_dict = {
                k: v.to(self.device) for k, v in val_data["edge_index_dict"].items()
            }
        else:
            x_dict = {
        ...
        with torch.no_grad():
            for pair in pairs:
                ...
                scores = self.model(
                    x_dict, edge_index_dict, user_ids, query_ids, llm_ids
                )
