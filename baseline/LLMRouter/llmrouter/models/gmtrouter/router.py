...
from llmrouter.models.gmtrouter.data_loader import (
    GMTRouterDataLoader,
    detect_data_format,
)
...
class GMTRouter(MetaRouter):
    ...
    def __init__(self, yaml_path: str):
        ...
        self.model_checkpoint_path = model_paths.get(
    ...
    def _load_pretrained_model(self):
        ...
        try:
            checkpoint = torch.load(
                self.model_checkpoint_path, map_location=self.device
            )
    ...
    def get_training_data(self) -> Tuple[Any, Any]:
        ...
        train_path = data_paths.get(
            "training_set", data_paths.get("routing_data_train")
        )
        ...
        if format_type != "gmtrouter":
            print(f"\n{'=' * 70}")
            print("ERROR: Data Format Mismatch!")
            print(f"{'=' * 70}")
            print("Expected: GMTRouter JSONL format")
            ...
            print()
            print(
                "See llmrouter/models/gmtrouter/README.md for complete documentation."
            )
            print(f"{'=' * 70}\n")
            return None, None
        ...
        if hasattr(train_graph, "x_dict"):
            ...
            train_data = {**train_graph, "train_metadata": train_metadata}
        ...
        if val_path and os.path.exists(val_path):
            ...
            if hasattr(val_graph, "x_dict"):
                ...
                val_data = {**val_graph, "val_metadata": val_metadata}
        else:
            ...
            val_metadata_split = {**train_metadata, "pairs": train_pairs[split_idx:]}
    ...
    def _initialize_model_from_data(self, data: Dict):
        ...
        print("GMTRouter model initialized and loaded from checkpoint")
    ...
    def route_single(self, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "x_dict" in self.graph_data:
            x_dict = {
                k: v.to(self.device) for k, v in self.graph_data["x_dict"].items()
            }
            edge_index_dict = {
                k: v.to(self.device)
                for k, v in self.graph_data["edge_index_dict"].items()
            }
        else:
            x_dict = {
        ...
        best_model_name = llm_id_to_name.get(
            best_llm_id, self.models[0] if self.models else "gpt-3.5-turbo"
        )
    ...
    def route_batch(
        self, batch: List[Dict[str, Any]], task_name: str = None
    ) -> List[Dict[str, Any]]:
