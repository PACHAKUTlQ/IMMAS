    ...
    try:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            if format_type == DataFormatType.GMTROUTER:
                ...
                print(
                    f"Warning: Unknown format detected in {file_path}, defaulting to standard"
                )
                return "standard"
...
class GMTRouterDataLoader:
                ...
                try:
                    ...
                    try:
                        ...
                        if validation_errors > 10:
                            print(
                                f"Too many validation errors (>{validation_errors}). Please check data format."
                            )
                            print("See GMTRouter README for correct JSONL format.")
    ...
    def _build_graph_from_interactions(self, interactions: List[Dict]):
        ...
        for interaction in interactions:
            ...
            if session_key not in self.session_map:
                ...
                # User-Session edges
                self.edges[HeteroEdges.USER_OWN_SESSION.value].append(
                    (user_id, self.num_sessions)
                )
                self.edges[HeteroEdges.SESSION_OWNED_BY_USER.value].append(
                ...
                if user_name in user_last_session:
                    ...
                    self.edges[HeteroEdges.SESSION_NEXT.value].append(
                        (prev_session, self.num_sessions)
                    )
                    self.edges[HeteroEdges.SESSION_PREV.value].append(
                        (self.num_sessions, prev_session)
                    )
            ...
            for conv_turn in conversation:
                ...
                if query_key not in self.query_map:
                    ...
                    # Session-Query edges
                    self.edges[HeteroEdges.SESSION_HAS_QUERY.value].append(
                        (session_id, self.num_queries)
                    )
                    self.edges[HeteroEdges.QUERY_IN_SESSION.value].append(
                        (self.num_queries, session_id)
                    )

                    # Temporal query edge
                    if session_key in session_last_query:
                        prev_query = session_last_query[session_key]
                        self.edges[HeteroEdges.QUERY_NEXT.value].append(
                            (prev_query, self.num_queries)
                        )
                        self.edges[HeteroEdges.QUERY_PREV.value].append(
                            (self.num_queries, prev_query)
                        )
                ...
                # Query-Response edges
                self.edges[HeteroEdges.QUERY_ANSWERED_BY_RESPONSE.value].append(
                    (query_id, response_id)
                )
                self.edges[HeteroEdges.RESPONSE_ANSWERED_TO_QUERY.value].append(
                    (response_id, query_id)
                )

                # LLM-Response edges
                self.edges[HeteroEdges.LLM_GENERATE_RESPONSE.value].append(
                    (llm_id, response_id)
                )
                self.edges[HeteroEdges.RESPONSE_GENERATED_BY_LLM.value].append(
                    (response_id, llm_id)
                )

                # User-LLM preference edge (based on rating)
                if rating >= 4.0:  # High rating threshold
                    self.edges[HeteroEdges.USER_PREFER_LLM.value].append(
                        (user_id, llm_id)
                    )
                    self.edges[HeteroEdges.LLM_PREFERRED_BY_USER.value].append(
                        (llm_id, user_id)
                    )

                # Response quality edges
                if rating >= 4.0:
                    self.edges[HeteroEdges.RESPONSE_HIGH_QUALITY.value].append(
                        (response_id, response_id)
                    )
                elif rating <= 2.0:
                    self.edges[HeteroEdges.RESPONSE_LOW_QUALITY.value].append(
                        (response_id, response_id)
                    )
        ...
        print(
            f"Graph built: {self.num_users} users, {self.num_sessions} sessions, "
    ...
    def _create_hetero_data(self):
        ...
        try:
            ...
            print(
                "Warning: PyTorch Geometric not installed. Returning simplified data."
            )
        ...
        if len(self.query_embeddings) > 0:
            data["query"].x = torch.tensor(
                np.array(self.query_embeddings), dtype=torch.float32
            )
        else:
        ...
        if len(self.llm_embeddings) > 0:
            data["llm"].x = torch.tensor(
                np.array(self.llm_embeddings), dtype=torch.float32
            )
        else:
        ...
        if len(response_features) > 0:
            ...
            data["response"].x = torch.zeros(
                (self.num_responses, self.embedding_dim or 128)
            )
    ...
    def _create_simplified_data(self):
        ...
        return {
            ...
            "session_embeddings": torch.zeros(
                (self.num_sessions, self.embedding_dim or 128)
            ),
            "query_embeddings": torch.tensor(
                np.array(self.query_embeddings), dtype=torch.float32
            )
            if self.query_embeddings
            else torch.zeros((self.num_queries, self.embedding_dim or 128)),
            "llm_embeddings": torch.tensor(
                np.array(self.llm_embeddings), dtype=torch.float32
            )
            if self.llm_embeddings
            else torch.zeros((self.num_llms, self.embedding_dim or 128)),
            "edges": self.edges,
