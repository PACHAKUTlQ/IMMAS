...
class HeteroGNN(nn.Module):
    ...
    def _forward_simplified(
        ...
        for i in range(self.num_layers):
            x_dict = {
                node_type: self.gnn_layers[i](x) for node_type, x in x_dict.items()
            }

            x_dict = {
                node_type: self.layer_norms[i](x) for node_type, x in x_dict.items()
            }
...
class PreferencePredictor(nn.Module):
    ...
    def forward(
        self, query_emb: torch.Tensor, user_emb: torch.Tensor, llm_emb: torch.Tensor
    ) -> torch.Tensor:
        """
