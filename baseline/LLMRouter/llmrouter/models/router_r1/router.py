...
def _get_tensor_parallel_size(model_id: str) -> int:
    ...
    try:
        ...
        print(
            f"Warning: failed to load model config for tensor parallel sizing: {e}. Using single GPU."
        )
        return 1
...
class RouterR1(MetaRouter):
    ...
    def route_single(self, query: Dict[str, Any], return_details: bool = False):
        ...
        sampling_params = SamplingParams(
            temperature=1.0, max_tokens=1024, stop=["</search>", "</answer>"]
        )
        ...
        while True:
            ...
            if tmp_query:
                ...
                try:
                    from llmrouter.models.router_r1.route_service import (
                        access_routing_pool,
                    )
                    ...
                    route_results = self.route(
                        tmp_query, api_base=self.api_base, api_key=self.api_key
                    )
            else:
                route_results = ""

            if not STOP:
                prompt += curr_route_template.format(
                    output_text=output_text, route_results=route_results
                )
                all_output += curr_route_template.format(
                    output_text=output_text, route_results=route_results
                )
            else:
        ...
        for row in query_data:
            ...
            try:
                result = self.route_single(
                    {"query": original_query}, return_details=True
                )
            ...
            row_copy["prompt_tokens"] = (
                prompt_tokens + route_tokens
            )  # Include routing in prompt cost
