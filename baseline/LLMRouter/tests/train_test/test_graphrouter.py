...
from llmrouter.models import GraphTrainer
...
def main():
    ...
    default_yaml = os.path.join(
        project_root, "configs", "model_config_train", "graphrouter.yaml"
    )

    parser = argparse.ArgumentParser(
        description="Train and test the GraphRouter with a YAML configuration file."
    )
    parser.add_argument(
        "--yaml_path",
        type=str,
