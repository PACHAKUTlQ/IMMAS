...
def main():
    ...
    default_yaml = os.path.join(
        project_root, "configs", "model_config_test", "gmtrouter.yaml"
    )
    ...
    try:
        for turn, query_text in enumerate(conversation_queries, start=1):
            ...
            print(
                f"  → {result.get('model_name', 'N/A')} (confidence: {result.get('confidence', 'N/A')})"
            )
    except Exception as e:
    ...
    try:
        batch = [
            {
                "query_text": "Solve 2+2",
                "user_id": "user_001",
                "session_id": "session_1",
                "turn": 1,
            },
            {
                "query_text": "Write a poem",
                "user_id": "user_001",
                "session_id": "session_1",
                "turn": 2,
            },
            {
                "query_text": "Debug this code",
                "user_id": "user_002",
