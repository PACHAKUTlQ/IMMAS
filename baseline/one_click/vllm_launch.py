...
def run(cmd: List[str], env: Optional[Dict[str, str]] = None, stdout_path: Optional[Path] = None) -> subprocess.Popen:
    stdout = None
...
def gpu_threshold_mb(
    ...
    if gpu_memory_utilization is not None:
        threshold = max(threshold, int(float(gpu_memory_utilization) * int(gpu_row["total_mb"])))
    return threshold
...
def main() -> None:
    ...
    for m in models:
        ...
        cmd = build_vllm_cmd("vllm_serve" if method == "vllm_serve" else "python_api_server", host, m)

        log_path = run_dir / f"vllm_{name}.log"
        print(f"[vllm_launch] Starting {name} on GPU={env.get('CUDA_VISIBLE_DEVICES','auto')} -> {log_path}")
