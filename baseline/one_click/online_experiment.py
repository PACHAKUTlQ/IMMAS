...
class JsonlDatasetSampler:
    ...
    def next_request(self) -> RequestItem:
        ...
        req_id = (
            row.get("request_id")
            or f"{self.name}-{self._idx}-{int(time.time() * 1000)}"
        )
        return RequestItem(
...
class WeightedDatasetMixer:
    ...
    def next_request(self) -> RequestItem:
        sampler = random.choices(
            [s for s, _ in self.samplers], weights=self.weights, k=1
        )[0]
...
class PerModelLimiter:
    def __init__(self, limits: Dict[str, int]):
        self.semaphores: Dict[str, asyncio.Semaphore] = {
            model_name: asyncio.Semaphore(max(1, int(limit)))
...
def build_candidates(
    cfg: Dict[str, Any], run_logger: logging.Logger
) -> Dict[str, Candidate]:
...
async def run_experiment(cfg: Dict[str, Any], run_dir: Path) -> None:
    ...
    if not os.environ.get("API_KEYS"):
        ...
        logger.info(
            "API_KEYS not set; defaulting API_KEYS from OPENAI_API_KEY (len=%d)",
            len(fallback_key),
        )
    ...
    limiter = PerModelLimiter(
        {name: c.per_model_max_concurrency for name, c in candidates.items()}
    )
    ...
    write_decisions = bool(
        cfg.get("output", {}).get("write_routing_decisions_jsonl", True)
    )
    write_metrics_csv = bool(cfg.get("output", {}).get("write_metrics_csv", True))

    req_f = (
        (run_dir / "requests.jsonl").open("w", encoding="utf-8")
        if write_requests
        else None
    )
    calls_f = (
        (run_dir / "model_calls.jsonl").open("w", encoding="utf-8")
        if write_calls
        else None
    )
    dec_f = (
        (run_dir / "routing_decisions.jsonl").open("w", encoding="utf-8")
        if write_decisions
        else None
    )
    metrics_f = (
        (run_dir / "metrics.csv").open("w", encoding="utf-8")
    ...
    token_cost_field = str(
        social_welfare_cfg.get("token_cost_field", "total_tokens")
    ).strip()
    ...
    async def flush(reason: str) -> None:
        ...
        logger.info(
            "Flush batch=%d size=%d reason=%s", batch_id, len(local_batch), reason
        )
        ...
        async def handle_one(req: RequestItem, enqueue_ms: float) -> None:
            ...
            if req_f:
                req_f.write(
                    json.dumps(dataclasses.asdict(req), ensure_ascii=False) + "\n"
                )

            t0 = now_ms()
            routing_result = await asyncio.to_thread(
                router.route_single, {"query": prompt}
            )
            t1 = now_ms()
            ...
            if chosen_model not in candidates:
                logger.warning(
                    "Chosen model '%s' not in candidates.json; skipping LLM call",
                    chosen_model,
                )
                return
            ...
            async def one_call(model_name: str, is_chosen: bool) -> None:
                ...
                try:
                    ...
                    total_tokens = int(
                        result.get("token_num") or (prompt_tokens + completion_tokens)
                    )
                    ...
                    if (
                        req.ground_truth is not None
                        and req.metric is not None
                        and req.metric != "online_only"
                    ):
                        try:
                    ...
                    if task_perf is not None:
                        social_welfare = float(task_perf) - (
                    ...
                    if is_chosen and task_perf is not None:
                        ...
                        per_model_chosen_task_perfs.setdefault(model_name, []).append(
                            float(task_perf)
                        )

                    if is_chosen:
                        chosen_total_tokens.append(int(total_tokens))
                        per_model_chosen_total_tokens.setdefault(model_name, []).append(
                            int(total_tokens)
                        )
                    ...
                    if calls_f:
                        calls_f.write(
                            json.dumps(dataclasses.asdict(mc), ensure_ascii=False)
                            + "\n"
                        )
                    ...
                    if is_chosen:
                        ...
                        per_model_chosen[model_name] = (
                            per_model_chosen.get(model_name, 0) + 1
                        )
                    else:
        ...
        await asyncio.gather(
            *(handle_one(req, enqueue_ms) for req, enqueue_ms in local_batch)
        )

        # Batch metrics row
        batch_end_ms = now_ms()
        queue_waits = [
            max(0.0, batch_start_ms - enqueue_ms) for _, enqueue_ms in local_batch
        ]
        ...
        if metrics_f:
            ...
            router_p50 = (
                percentile(router_latencies[-len(local_batch) :], 0.50)
                if router_latencies
                else float("nan")
            )
            chosen_llm_p50 = (
                percentile(chosen_llm_latencies[-len(local_batch) :], 0.50)
                if chosen_llm_latencies
                else float("nan")
            )
            chosen_success_rate = 1.0 - (
                batch_chosen_errors / max(1, batch_chosen_calls)
            )

            recent_perf = (
                chosen_task_perfs[-batch_chosen_calls:]
                if batch_chosen_calls > 0
                else []
            )
            recent_tokens = (
                chosen_total_tokens[-batch_chosen_calls:]
                if batch_chosen_calls > 0
                else []
            )
            recent_welfare = (
                chosen_social_welfares[-len(recent_perf) :] if recent_perf else []
            )
            perf_p50 = (
                percentile([float(x) for x in recent_perf], 0.50)
                if recent_perf
                else float("nan")
            )
            tokens_p50 = (
                percentile([float(x) for x in recent_tokens], 0.50)
                if recent_tokens
                else float("nan")
            )
            welfare_p50 = (
                percentile([float(x) for x in recent_welfare], 0.50)
                if recent_welfare
                else float("nan")
            )
            metrics_f.write(
    ...
    async for _ in arrivals:
        ...
        if (
            buffer
            and batch_open_ms is not None
            and (now_ms() - batch_open_ms) >= time_window_ms
        ):
            await flush("time")
            batch_open_ms = None
