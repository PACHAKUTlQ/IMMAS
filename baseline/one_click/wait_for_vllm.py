...
def main() -> None:
    ...
    while pending and time.time() < deadline:
        ...
        for idx, m in enumerate(models):
            ...
            if not is_pid_alive(pid):
                name = str(m.get("name") or m.get("served_model_name") or f"model_{idx}")
                log_path = run_dir / f"vllm_{name}.log"
                print(f"[wait_for_vllm] vLLM process exited early: name={name} pid={pid}", file=sys.stderr)
                if log_path.exists():
                    print(f"[wait_for_vllm] ---- tail {log_path} ----", file=sys.stderr)
                    print(tail_text(log_path), file=sys.stderr)
                else:
                    print(f"[wait_for_vllm] Log file not found: {log_path}", file=sys.stderr)
                raise SystemExit(1)
    ...
    if pending:
        raise SystemExit(f"vLLM health check timed out. Still pending: {sorted(pending)}")
