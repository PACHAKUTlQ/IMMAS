...
from immas.data.quac.loader import QUAC_CONFIG_NAME, QUAC_DATASET_NAME, QuacDialogue
...
def _make_initial_messages(dialogue: QuacDialogue) -> List[Dict[str, Any]]:
    ...
    system = {
        "role": "system",
        "content": """
You are answering a student's questions about a hidden Wikipedia section.

Rules:
- Use ONLY the provided Background + Section Context.
- The answer MUST be a verbatim span copied from the Context (no paraphrase),
  unless the question cannot be answered from the Context.
- If it cannot be answered from the Context, output exactly: CANNOTANSWER

Output format:
- You may think step-by-step, but the final answer MUST be the last non-empty line.
- The final answer line must contain only the span (or CANNOTANSWER), with no extra words.
""",
    }
    ...
    context_msg = {
        "role": "user",
        "content": (
            f"Wikipedia page: {dialogue.wikipedia_page_title}\n"
            f"Dialogue id: {dialogue.dialogue_id}\n\n"
            f"Background:\n{dialogue.background}\n\n"
            f"Section title:\n{dialogue.section_title}\n\n"
            f"Section context:\n{dialogue.context}\n"
        ),
    }
...
async def main_async() -> None:
    ...
    for i, ex in enumerate(ds):
        if i >= max_dialogues:
            break
        dialogues.append(QuacDialogue.from_hf_example(ex))
